...
fields_MCMCresults = [cellfun(@(x) strcat('mean_',x), varparams, 'UniformOutput', false),...
    cellfun(@(x) strcat('CI_',x), varparams, 'UniformOutput', false),{'cell_index','ApprovedFits','mcmcrun'}]; %Add prefixes 'mean_' and 'CI_' to all parameter names. Further add the fields 'cell_index', 'ApprovedFits' and 'mcmcrun'.
