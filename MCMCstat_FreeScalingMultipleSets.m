function MCMCstat_FreeScalingMultipleSets(n_burn, n_steps, ncstring, analysistype, varargin)

%Fits constant elongation model with dwell time to 2-color data using MCMCstat package. The script
%loads multiple dataset and finds a probability distribution for the parameters
%using Markov Chain Monte Carlo, for each single trace. The initial parameter values are randomly chosen
%within a reasonable range. This model assumes that the calibration factor
%between MS2 and PP7 is unknown and attempts to fit it. This script only
%looks at the initial rise in the nuclear cycle and assumes a dwell time
...
%file for each dataset, with each single nucleus saved inside a structure
%file.
%The inputs are:

%   n_chains: number of MCMC chains
%   n_burn: number of burn-in steps in MCMC algorithm
%   n_steps: number of steps in MCMC algorithm
%   ncstring: which nuclear cycle we're looking at ('nc13' or 'nc14')
%   analysistype: 'initial' - look at initial rise in trace
%                 'whole' - look at whole cycle
%                 'whole_using_initial' - look at whole cycle using
%                 inferred v_elon and R0 from initial rise inference
%Variable inputs (Name/Value pairs):
%   'PolyRate', n: polynomial rate approximation of nth order (follow this
%                  with the order n). This approximation takes the form:
%                     R = R0 + R1*t + ... + Rn*t^n
%   'MeanRate', s: fit rate around a constant mean value with fluctuations at
%                  each timepoint, where the fluctuations have a prior
%                  centered around zero with variance s.
%   'ModelType', modeltype: Choose model type
%                       'Dwell' - Fixed dwell time model
%                       'Termination' - Fixed termination rate model
%   'Construct', construct: Choose construct
%                       'P2P-MS2v5-LacZ-PP7v4' - base construct
%                       'P2P-MS2v5-LacZshort-PP7v4' - short construct
%                       'P2P-MS2v5-LacZlong-PP7v4' - long construct
%                       'P2P-MS2v5-Tub3UTR-PP7v4-Tub3UTR' - original
%                       tubulin 3'UTR construct
%                       'P2P-MS2v5-LacZ-601-PP7v4' - 601 NPS construct
